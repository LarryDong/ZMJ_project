...
void process() {
	...
	ROS_WARN("--> (ver) Mapping node begin...");
	while(1){
...
int main(int argc, char **argv){
	...
	ros::Subscriber subLaserCloudCornerLast = nh.subscribe<sensor_msgs::PointCloud2>("/vlaser_cloud_corner_last", 100, laserCloudCornerLastHandler);
    ros::Subscriber subLaserCloudSurfLast = nh.subscribe<sensor_msgs::PointCloud2>("/vlaser_cloud_surf_last", 100, laserCloudSurfLastHandler);
	ros::Subscriber subLaserOdometry = nh.subscribe<nav_msgs::Odometry>("/vlaser_odom_to_init", 100, laserOdometryHandler);
	ros::Subscriber subLaserCloudFullRes = nh.subscribe<sensor_msgs::PointCloud2>("/vlslidar_point_cloud_3", 100, laserCloudFullResHandler);
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// pubLaserCloudSurround = nh.advertise<sensor_msgs::PointCloud2>("/laser_cloud_surround", 100);
	pubLaserCloudMap = nh.advertise<sensor_msgs::PointCloud2>("/vlaser_cloud_map", 100);
	pubLaserCloudFullRes = nh.advertise<sensor_msgs::PointCloud2>("/vlslidar_point_cloud_registered", 100);
	pubOdomAftMapped = nh.advertise<nav_msgs::Odometry>("/vaft_mapped_to_init", 100);
	pubOdomAftMappedHighFrec = nh.advertise<nav_msgs::Odometry>("/vaft_mapped_to_init_high_frec", 100);
	pubLaserAfterMappedPath = nh.advertise<nav_msgs::Path>("/vaft_mapped_path", 100);
	for (int i = 0; i < laserCloudNum; i++){
