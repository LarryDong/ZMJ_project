...
std::queue<sensor_msgs::PointCloud2ConstPtr> cornerSharpBuf, vcornerSharpBuf;
std::queue<sensor_msgs::PointCloud2ConstPtr> cornerLessSharpBuf, vcornerLessSharpBuf;
std::queue<sensor_msgs::PointCloud2ConstPtr> surfFlatBuf, surfFlatBuf;
std::queue<sensor_msgs::PointCloud2ConstPtr> surfLessFlatBuf, surfLessFlatBuf;
std::queue<sensor_msgs::PointCloud2ConstPtr> fullPointsBuf, fullPointsBuf;
std::mutex mBuf;
...
// vertical lidar handler
void vlaserCloudSharpHandler(const sensor_msgs::PointCloud2ConstPtr &cornerPointsSharp2){
#ifdef DEBUG_OUTPUT
    printf("[DEBUG] vSharp callback \n");
#endif
    mBuf.lock();
    cornerSharpBuf.push(cornerPointsSharp2);
    mBuf.unlock();
}
void vlaserCloudLessSharpHandler(const sensor_msgs::PointCloud2ConstPtr &cornerPointsLessSharp2){
#ifdef DEBUG_OUTPUT
    printf("[DEBUG] vlessSharp callback \n");
#endif
    mBuf.lock();
    cornerLessSharpBuf.push(cornerPointsLessSharp2);
    mBuf.unlock();
}
void vlaserCloudFlatHandler(const sensor_msgs::PointCloud2ConstPtr &surfPointsFlat2){
#ifdef DEBUG_OUTPUT
    printf("[DEBUG] vFlat callback \n");
#endif
    mBuf.lock();
    surfFlatBuf.push(surfPointsFlat2);
    mBuf.unlock();
}
void vlaserCloudLessFlatHandler(const sensor_msgs::PointCloud2ConstPtr &surfPointsLessFlat2){
#ifdef DEBUG_OUTPUT
    printf("[DEBUG] vlessFlat callback \n");
#endif
    mBuf.lock();
    surfLessFlatBuf.push(surfPointsLessFlat2);
    mBuf.unlock();
}
void vlaserCloudFullResHandler(const sensor_msgs::PointCloud2ConstPtr &laserCloudFullRes2){
    mBuf.lock();
    fullPointsBuf.push(laserCloudFullRes2);
    mBuf.unlock();
}
...
int main(int argc, char **argv){
    ...
    // vertical lidar
    ros::Subscriber subLaserCloudFullRes = nh.subscribe<sensor_msgs::PointCloud2>("/vlslidar_point_cloud_2", 100, vlaserCloudFullResHandler);
    ros::Subscriber subCornerPointsSharp = nh.subscribe<sensor_msgs::PointCloud2>("/vlaser_cloud_sharp", 100, vlaserCloudSharpHandler);
    ros::Subscriber subCornerPointsLessSharp = nh.subscribe<sensor_msgs::PointCloud2>("/vlaser_cloud_less_sharp", 100, vlaserCloudLessSharpHandler);
    ros::Subscriber subSurfPointsFlat = nh.subscribe<sensor_msgs::PointCloud2>("/vlaser_cloud_flat", 100, vlaserCloudFlatHandler);
    ros::Subscriber subSurfPointsLessFlat = nh.subscribe<sensor_msgs::PointCloud2>("/vlaser_cloud_less_flat", 100, vlaserCloudLessFlatHandler);
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
