...
//~ point to line optimization.
//~ a: nearest point to current point in last cloud; b: nearest point to a in last point on a different scan.
struct LidarEdgeFactor{
	...
	bool operator()(const T *q, const T *t, T *residual) const{
        ...
        //~ calculate point 2 line distance. Using parallemlogram's area. S = vec{a} \cross vec{b} = bottom * height 
		Eigen::Matrix<T, 3, 1> nu = (lp - lpa).cross(lp - lpb); //~ numerator: S_parallemlogram
		Eigen::Matrix<T, 3, 1> de = lpa - lpb;                  //~ denominator: bottom length.
		residual[0] = nu.x() / de.norm();       //~ TODO: why 3 parts?
...
//~ point to plan optimization.
//~ j: nearest point in last, l/m: nearest to point j on same/different scan. jlm: a plane.
struct LidarPlaneFactor{
        ...
        : curr_point(curr_point_), last_point_j(last_point_j_), last_point_l(last_point_l_),last_point_m(last_point_m_), s(s_) 
    {
		...
		ljm_norm.normalize();           //~ the plane's norm.
	}
	...
	bool operator()(const T *q, const T *t, T *residual) const{
		...
		residual[0] = (lp - lpj).dot(ljm);      //~ point-to-plane distance.
